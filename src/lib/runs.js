...
const GAP_ENDS_RUN = 3; // days between completions that kills a run
...
export function splitIntoRuns(completions, todayDate = localDateStr(new Date())) {
  ...
  return runs.map((r, idx) => {
    ...
    // A run is alive only if it can still be EXTENDED. You may log today or
    // back-fill yesterday, so the furthest a live run's last act can sit is
    // 3 days back: filling yesterday would make the gap 2, which survives.
    // 4+ days back and there is no legal move left -- the run is over.
    const isAlive = isLast && dayDiff(endDate, todayDate) <= 3;
